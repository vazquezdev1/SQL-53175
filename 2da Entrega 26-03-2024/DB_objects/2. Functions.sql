...
-- FUNCIÓN 2
delimiter //
create function `fn_calculate_salaryPerDay` (_salaryPerHour float, _workedHoursPerWeek int, _xDays int) 
	returns float 
    no sql
    begin
		declare _workedHoursPerDay int;
		declare _salaryPerXDays float;
        set _workedHoursPerDay = round(_workedHoursPerWeek / 6); -- Calculamos el aproximado de horas trabajadas en un día
